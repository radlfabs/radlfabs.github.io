...
Let's copy the files to another directory, where we use the models in a REST API. `Flask`is a great choice to build a REST API with our trained models in a super simple way. We will write to files: `app.py` and `client.py`. Here is what we need inside of `app.py`:
